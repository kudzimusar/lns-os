...
export default function TeacherMarkingSubmissionsPage({ params }: { params: { id: string, studentId: string } }) {
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100 overflow-y-auto">
      <div className="p-8 border-b border-gray-50 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-md z-10">
         <div className="flex items-center gap-4">
            <Link href={`/teacher/assignments/${params.id}/submissions`}>
               <Button variant="ghost" size="icon" className="rounded-full hover:bg-gray-100 text-lns-navy border border-gray-100 h-10 w-10 shadow-sm">
                  <ArrowLeft size={18} />
               </Button>
            </Link>
    ...
    <div className="h-full bg-gray-50 flex flex-col">
       <div className="p-8 border-b border-gray-100 bg-white/95 backdrop-blur-md sticky top-0 z-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <div className="flex flex-col">
                <h3 className="text-sm font-black text-lns-navy uppercase tracking-tight leading-none mb-1 italic">Asset Node Visualization</h3>
          ...
          <div className="flex items-center gap-4">
             <Button variant="ghost" size="icon" className="h-10 w-10 text-lns-mid-grey hover:bg-white hover:shadow-xl rounded-xl transition-all">
                <ScanEye size={20} />
             </Button>
             <Button variant="ghost" size="icon" className="h-10 w-10 text-lns-mid-grey hover:bg-white hover:shadow-xl rounded-xl transition-all">
                <TrendingUp size={20} />
       ...
       <div className="flex-1 p-12 flex items-center justify-center overflow-y-auto pb-48">
          <Card className="w-full h-fit max-w-5xl bg-white border-none shadow-[0_50px_100px_rgba(0,0,0,0.08)] rounded-[0.5rem] p-24 space-y-16 animate-in zoom-in-95 duration-700">
  ...
  return (
    <div className="h-[calc(100vh-140px)] -m-4 md:-m-6 lg:-m-8">
      <SplitPanel 
        ...
        showMobileDetail={false} // Focusing on density for marking
